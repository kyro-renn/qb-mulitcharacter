...
QBCore.Functions.CreateCallback("qb-multicharacter:server:GetNumberOfCharacters", function(source, cb)
    local src = source
    local license = QBCore.Functions.GetIdentifier(src, 'license')
    local numOfChars = Config.DefaultNumberOfCharacters -- Default value if no role matches

    -- Query to get the membership role for the player from the database
    MySQL.Async.fetchAll('SELECT * FROM kyro_memberships WHERE license = @license AND expire_date > NOW()', {
        ['@license'] = license
    }, function(memberships)
        if memberships and #memberships > 0 then
            -- Iterate through all active memberships
            for _, membership in ipairs(memberships) do
                -- Check the role of the membership and set the corresponding number of characters
                local role = membership.member
                if Config.Roles[role] then
                    numOfChars = Config.Roles[role].numOfChars
                    break
                end
            end
        end
        
        -- Return the determined number of characters
        cb(numOfChars)
    end)
end)
